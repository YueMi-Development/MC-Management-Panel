...
public class WipeCommand implements SubCommand {

    private final ManagementPanelPlugin plugin;

    public WipeCommand(ManagementPanelPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public void execute(@NotNull CommandSender sender, @NotNull String[] args) {
        if (args.length < 2) {
            CommandHelper.sendMsg(sender, "Usage: /mp wipe <player>", NamedTextColor.RED);
            return;
        }
        
        String targetName = args[1];
        CommandHelper.sendMsg(sender, "Resolving target and starting wipe process for: " + targetName, NamedTextColor.YELLOW);
        ...
        CompletableFuture.runAsync(() -> {
            ...
            plugin.getWipeServiceImpl().wipe(op.getUniqueId())
                    ...
                    .exceptionally(t -> {
                        CommandHelper.sendMsg(sender, "Failed to wipe player " + targetName + ": " + t.getCause().getMessage(), NamedTextColor.RED);
                        return null;
                    });
        });
    }
